...
public class ObservationEvaluator {
    public ObservationEvaluation evaluate(WorkerDescriptor worker, TaskNode node, Observation observation) {
        ...
        boolean grounded = !requiresGrounding(worker) || hasEvidence;
        ...
        if (!grounded) {
            reasons.add("Worker output needs evidence or artifacts but none were produced.");
        }
        ...
        boolean needsFallback = failed && observation.retryable() && !worker.fallbackWorkerNames().isEmpty();
        boolean needsReplan = failed && !observation.retryable() && worker.fallbackWorkerNames().isEmpty();
        return new ObservationEvaluation(observation.id(), observation.taskNodeId(), observation.workerName(),
    ...
    private boolean requiresGrounding(WorkerDescriptor worker) {
        ...
        return type.contains("query") || type.contains("proposal") || worker.outputContract().stream()
